...
Each line is the read in and converted to hash of :header => value which is added to an array. 
...
Admitedly this is fairly inefficient, however, this is a hacky tool designed for hacky jobs :)
...
It's all pretty simply but you can do nice things like:

   data.select { |d| d[:Age].to_i > 15 }.columns(:Location).uniq

to get a list of the locations where people are above certain age. 
